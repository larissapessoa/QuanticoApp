...
export class Level7Page implements OnInit {
  ...
  public showTime;   // Time shown as string format
  ...
  public imageDir = '../assets/img/desafio7/';
  ...
  selectCard(pos, val, i){
    ...
    // if we have both cards selected, check for matcg on fail
  ...
  restartGame(){
    ...
    this.showTime = 0;   // Time shown as string format
    this.countTime = 0;
    this.interCount = null; //Timer: 1 second for in game counter
    this.userLife = 10; 
    this.resetSelects();
    this.populateCards();
    this.shuflle(this.cardsArray);
    this.shuflle(this.images);
    ...
    // this will substract 1 from countdown start time
    ...
    setTimeout(() => {
      this.interTime = setInterval(() => {
        ...
        if (this.gameState == 'init'){
          ...
          this.showTime = minutes.toString() + ":" + seconds.toString();
  ...
  winCon() {
    var winCheck = false;
    // If at least 1 or more cards have not been salved
  ...
  // fuction to reset selected cards

  resetSelects() {
  this.selectCard1pos = -1; // Selected card #1 position
  this.selectCard1val = -1; //Selected card #1 
